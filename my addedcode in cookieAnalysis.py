...
if __name__ == "__main__":
    ...
    n, bins, patches = plt.hist(times, 100, density=True, facecolor='purple', edgecolor= 'b', alpha=0.75)
    ...
    plt.text(quarlocate[0],0.80, quarperc[0], color='y')
    plt.text(quarlocate[1],0.70, quarperc[1], color='y')
    plt.text(quarlocate[2],0.60, quarperc[2], color='y')
    plt.text(quarlocate[3],0.50, quarperc[3], color='y')
    
    #quartile lines
    for q in quarlocate:
        plt.axvline(q ,color='g')
        plt.text(q, -.10, round(q, 2), color='y', rotation= 90)
